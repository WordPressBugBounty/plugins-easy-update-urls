...
function easy_update_urls_sysinfo_get()
{
    ...
    $return .= 'Version:                  ' . EASY_UPDATE_URLS_VERSION;
    $return .= "\n\n";
    $return .= '-- Site Info' . "\n\n";
    ...
    try {
        $easy_update_urls_cpu_info = easy_update_urls_get_full_cpu_info();
        $cpu_section_written = false;

        if (!empty($easy_update_urls_cpu_info['cores']) && $easy_update_urls_cpu_info['cores'] !== 'Unknown') {
            if (!$cpu_section_written) {
                $return .= "\n-- CPU Information\n\n";
                $cpu_section_written = true;
            }
            $return .= 'Number of Cores:          ' . $easy_update_urls_cpu_info['cores'] . "\n";
        }

        if (!empty($easy_update_urls_cpu_info['architecture']) && $easy_update_urls_cpu_info['architecture'] !== 'Unknown') {
            if (!$cpu_section_written) {
                $return .= "\n-- CPU Information\n\n";
                $cpu_section_written = true;
            }
            $return .= 'Architecture:             ' . $easy_update_urls_cpu_info['architecture'] . "\n";
        }

        if (!empty($easy_update_urls_cpu_info['model']) && $easy_update_urls_cpu_info['model'] !== 'Unknown') {
            if (!$cpu_section_written) {
                $return .= "\n-- CPU Information\n\n";
                $cpu_section_written = true;
            }
            $return .= 'Model:                    ' . $easy_update_urls_cpu_info['model'] . "\n";
        }

        // Load Averages
        //$easy_update_urls_load = easy_update_urls_get_load_averages();
        //easy_update_urls_get_load_average
        $easy_update_urls_load = easy_update_urls_get_load_average();
        //easy_update_urls_calculate_load_percentage
        $easy_update_urls_cores = is_numeric($easy_update_urls_cpu_info['cores']) ? (int)$easy_update_urls_cpu_info['cores'] : 1;
        if (!empty($easy_update_urls_load)) {
            $return .= "\n-- System Load Averages\n\n";
            foreach (['1min', '5min', '15min'] as $interval) {
                $value = $easy_update_urls_load[$interval] ?? null;
                $percent = easy_update_urls_calculate_load_percentage($value, $easy_update_urls_cores);
    ...
    $return .= "\n" . '-- User Browser' . "\n\n";
    $return .= $easy_update_urls_userAgentOri; // $browser;
    $return .= "\n\n";
    $locale = get_locale();
...
function easy_update_urls_OSName()
{
    try {
...
function easy_update_urls_get_host()
{
 ...
 */
function easy_update_urls_get_load_average()
{
    try {
        // Attempt to use sys_getloadavg()
        if (function_exists('sys_getloadavg')) {
            $easy_update_urls_load = sys_getloadavg();
            if ($easy_update_urls_load !== false && is_array($easy_update_urls_load)) {
                return [
                    '1min'  => $easy_update_urls_load[0],
                    '5min'  => $easy_update_urls_load[1],
                    '15min' => $easy_update_urls_load[2],
                ];
            }
        }

        // Fallback to reading /proc/loadavg
        return easy_update_urls_get_load_average_from_proc();
    } catch (Exception $e) {
        return [
            '1min'  => null,
            '5min'  => null,
 ...
 * @return array Load averages for 1, 5, and 15 minutes
 */
function easy_update_urls_get_load_average_from_proc()
{
    try {
        if (file_exists('/proc/loadavg')) {
            $easy_update_urls_contents = @file_get_contents('/proc/loadavg');
            if ($easy_update_urls_contents !== false) {
                $easy_update_urls_parts = explode(' ', trim($easy_update_urls_contents));
                if (count($easy_update_urls_parts) >= 3) {
                    return [
                        '1min'  => (float) $easy_update_urls_parts[0],
                        '5min'  => (float) $easy_update_urls_parts[1],
                        '15min' => (float) $easy_update_urls_parts[2],
                    ];
                }
            }
        }
 ...
 */
function easy_update_urls_get_cpu_cores()
{
    $easy_update_urls_cores = false;
    ...
    // Método 2: system()
    if ($easy_update_urls_cores === false && function_exists('system')) {
        try {
            ob_start();
            @system('nproc --all');
    ...
    // Método 3: passthru()
    if ($easy_update_urls_cores === false && function_exists('passthru')) {
        try {
            ob_start();
            @passthru('nproc --all');
            $output = trim(ob_get_clean());
    ...
    // Método 4: popen()
    if ($easy_update_urls_cores === false && function_exists('popen')) {
        try {
    ...
    // Método 5: proc_open()
    if ($easy_update_urls_cores === false && function_exists('proc_open')) {
        try {
            $descriptorspec = [
                1 => ['pipe', 'w']
            ];
    ...
    if ($easy_update_urls_cores === false) {
        try {
    ...
    if ($easy_update_urls_cores === false && is_readable('/proc/cpuinfo')) {
        try {
 ...
 */
function easy_update_urls_get_full_cpu_info()
{
    $easy_update_urls_info = [
        'cores' => null,
        'architecture' => null,
        'model' => null,
    ];
    ...
    try {
        // 1. Get cores
        $easy_update_urls_cores = easy_update_urls_get_cpu_cores();
        if (is_numeric($easy_update_urls_cores)) {
            $easy_update_urls_info['cores'] = $easy_update_urls_cores;
        } else {
            $easy_update_urls_info['cores'] = 'Unknown';
        }

        // 2. Get architecture
        try {
            $easy_update_urls_info['architecture'] = php_uname('m') ?: 'Unknown';
        } catch (Exception $e) {
            $easy_update_urls_info['architecture'] = 'Unknown';
        }
        ...
        if (file_exists('/proc/cpuinfo') && is_readable('/proc/cpuinfo')) {
            try {
                $easy_update_urls_cpuinfo = @file_get_contents('/proc/cpuinfo');
                if ($easy_update_urls_cpuinfo !== false && preg_match('/model name\s+:\s+(.+)/', $easy_update_urls_cpuinfo, $matches)) {
                    $easy_update_urls_info['model'] = trim($matches[1]);
            ...
            if (!empty($output)) {
                foreach ($output as $line) {
                    if (stripos($line, 'Model name:') === 0) {
                        $easy_update_urls_info['model'] = trim(substr($line, strpos($line, ':') + 1));
                        $cpu_model_found = true;
                        break;
                    }
                }
        ...
        if (!$cpu_model_found && function_exists('shell_exec') && stripos(PHP_OS, 'Darwin') === 0) {
            ...
            if (!empty($sysctl_output)) {
                $easy_update_urls_info['model'] = trim($sysctl_output);
                $cpu_model_found = true;
            }
        }
        ...
        // Final fallback
        if (!$cpu_model_found) {
            $easy_update_urls_info['model'] = 'Unknown';
        }

        return $easy_update_urls_info;
    } catch (Exception $e) {
        return $easy_update_urls_info;
    }
}
 ...
 */
function easy_update_urls_calculate_load_percentage($load, $cores)
{
    try {
