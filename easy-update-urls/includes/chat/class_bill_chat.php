...
namespace easy_update_urls_BillChat;
// 2024-12=18
if (!defined('ABSPATH')) {
    die('Invalid request.');
}
...
class ChatPlugin
{
    ...
    public function enqueue_chat_scripts()
    {
        ...
        wp_localize_script('chat-script', 'bill_data', array(
            ...
            'reset_success'            => esc_attr__('Chat messages reset successfully.', 'easy-update-urls'),
            'reset_error'              => esc_attr__('Error resetting chat messages.', 'easy-update-urls'),
            'invalid_message'          => esc_attr__('Invalid message received:', 'easy-update-urls'),
            'invalid_response_format'  => esc_attr__('Invalid response format:', 'easy-update-urls'),
            'response_processing_error' => esc_attr__('Error processing server response:', 'easy-update-urls'),
            'not_json'                 => esc_attr__('Response is not valid JSON.', 'easy-update-urls'),
            'ajax_error'               => esc_attr__('AJAX request failed:', 'easy-update-urls'),
            'send_error'               => esc_attr__('Error sending the message. Please try again later.', 'easy-update-urls'),
            'empty_message_error'      => esc_attr__('Please enter a message!', 'easy-update-urls'),
        ));
    }
    /**
    ...
    public function bill_chat_call_chatgpt_api($data)
    {
        ...
        if (false === get_transient($transient_name)) {
            ...
            try {
                ...
                include_once EASY_UPDATE_URLS_PATH . 'dashboard/tools.php';
        ...
        $data2 = [
            'param1' => $data,
            'param2' => $easy_update_urls_checkup,
        ...
        if (isset($data['success']) && $data['success'] === true) {
            ...
            $message = esc_attr__("Error contacting the Artificial Intelligence (API). Please try again later.", "easy-update-urls");
        }
        return $message;
    }
    /**
    ...
    public function bill_chat_send_message()
    {
        ...
        if (!empty($response_data)) {
            ...
            $output = esc_attr__("Error to get response from AI source!", "easy-update-urls");
        }
